...
- Subject: `VPS HTTPS pilot deployment`
...
- Next engineering gate: deploy the approved immutable release after secure VPS environment preparation
- Waiting human test: no
- Waiting release approval: no
- Last verified checkpoint: old inactive VPS archive `/root/HomeQuranLearning.QA.zip` was deleted by exact path after read-only inventory; no active containers, volumes, services or project directory were present
...
- VPS TLS diagnosis/fix checkpoint (2026-08-29): External Windows PowerShell/curl/Chrome clients failed before HTTP with `tlsv1 alert internal error`, while an OpenSSL probe with explicit SNI completed TLS and returned the expected Caddy 403 from the non-allowlisted Docker source. Caddy access logs showed no external HTTPS request, and its adapted config had no default for empty SNI. The documented `default_sni {$ACADEMY_HOST}` fix is now applied to the checked-in Caddyfile and its generator; local Caddy validation and PowerShell parsing pass. Remote VPS has not been changed yet.
