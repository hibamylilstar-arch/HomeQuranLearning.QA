...
The system is for internal use on academy-controlled devices only — it is
not a commercial product and does not need to support arbitrary third-party
tenants.

## Decision

1. **Three-tier system**: Windows Agent (teacher laptops) → Cloud Backend →
...
are never stored directly in the database — only metadata is. This keeps
...
5. **Tiered QA detection** (keyword → phrase/context → optional AI
...
target, not Kubernetes — reflecting current scale and operational
...
pipeline (capture → record → store → play back → transcribe → alert) on
...
* **Kubernetes / microservices from day one** — rejected as unnecessary
operational overhead at \~20-device scale.
* **Storing recordings in PostgreSQL** — rejected due to database bloat and
backup/performance implications at scale.
* **Plain HTTP/MJPEG live streaming** — rejected due to latency unsuitable
for real-time monitoring.
* **Keyword-only QA matching** — rejected due to high false-positive risk
